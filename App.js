...
// The city you want to monitor
const CITY_NAME = 'your city';

// The wind speed threshold for sending a notification (in meters per second)
const WIND_SPEED_THRESHOLD = 3.0;
...
// Function to check wind speed and send a notification if necessary
async function checkWindSpeed() {
    const weatherData = await fetchWeatherData(CITY_NAME);
    const windSpeed = weatherData.wind.speed;

    if (windSpeed > WIND_SPEED_THRESHOLD) {
        // If the wind speed exceeds the threshold, send a notification
        // This is a simple console log, but you could replace this with code to send a push notification
        console.log(`Wind speed in ${CITY_NAME} has exceeded the threshold! Current speed: ${windSpeed} m/s`);
    }
}

// Call the function to start checking wind speed
checkWindSpeed();
